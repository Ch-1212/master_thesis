...
results_feature = pd.read_csv(r'data/Friedrichshagen/result_feature_training.csv')
results_kmeans_dtw = pd.read_csv(r'data/Friedrichshagen/result_kmeans_training.csv')
results_sax_kmeans = pd.read_csv(r'data/saxpy_results/result_sax_kmeans_paa_12_alph_3_training.csv')
results_sax_kmeans = results_sax_kmeans[:len(results_kmeans_dtw)]

anomalie_1_kmeans_dtw = pd.read_csv(r'data/Friedrichshagen/anomalie_1_kmeans_dtw.csv')
anomalie_2_kmeans_dtw = pd.read_csv(r'data/Friedrichshagen/anomalie_2_kmeans_dtw.csv')
anomalie_1_feature = pd.read_csv(r'data/Friedrichshagen/anomalie_1_feature.csv')
anomalie_2_feature = pd.read_csv(r'data/Friedrichshagen/anomalie_2_feature.csv')

# Plot original data with clustering results
uplot.plot_clustered_data(np.array(results_feature))
uplot.plot_clustered_data_with_days(np.array(results_kmeans_dtw))
uplot.plot_clustered_data(np.array(results_sax_kmeans))
#uplot.plot_clustered_data_with_days(np.array(results_sax_kmeans))
#uplot.plot_clustered_data(results_kmeans_dtw)
uplot.plot_clustered_data_with_anomalies(np.array(results_kmeans_dtw), anomalie_1_kmeans_dtw, anomalie_2_kmeans_dtw)
...
ari_dtw_kmeans = adjusted_rand_score(own_labels[:len(results_kmeans_dtw.dtw_labels)], results_kmeans_dtw.dtw_labels)
...
ami_kmeans_dtw = adjusted_mutual_info_score(own_labels[:len(results_kmeans_dtw.dtw_labels)], results_kmeans_dtw.dtw_labels)
...
fowlkes_mallows_kmeans = fowlkes_mallows_score(own_labels[:len(results_kmeans_dtw.dtw_labels)], results_kmeans_dtw.dtw_labels)
...
#Feature - DTW
contingency_tbl = pd.crosstab(results_feature.feature_labels, results_kmeans_dtw.dtw_labels)
...
#DTW - SAX
contingency_tbl = pd.crosstab(results_kmeans_dtw.dtw_labels, results_sax_kmeans.sax_labels)
probablity_tbl = contingency_tbl/ contingency_tbl.sum()
plt.figure()
sns.heatmap(probablity_tbl, annot=True, center=0.5 ,cmap="Greys")
plt.show()
