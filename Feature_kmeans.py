...
df_feature_sel = sel.fit_transform(df_feature) # Würde Slope kicken, weil der bei allen ziemlich gleich ist!

# Standardize features
df_feature_z = z_score_standardization(df_feature)
#df_feature_z.to_csv(r'data/data_features_training.csv')
...
def kmeans_feature(feature):
    ...
    #joblib.dump(k_means, "data/fitted_models/feature_kmeans.pkl")

    # Cluster membership
    _labels = pd.Series(k_means.labels_, index=df_feature_z.index)

    print('Finished k_means sklearn')

    # Only works for two features
    #plot_feature_result(X_train, feature, kmeans, labels)
    ...
    #data_labels.to_csv(r'data/Friedrichshagen/result_feature_training.csv')

    # Detect anomalies
    #anomalies_indexes_1 = ad.detect_anomalie_1(data_labels, k_means.labels_)
    #anomalies_indexes_2 = ad.detect_feature_anomalie(data_labels, x_train, k_means)

    # anomalies_indexes_1 = pd.DataFrame(anomalies_indexes_1)
    # anomalies_indexes_1.to_csv(r'data/anomalie_1_feature.csv')
    # anomalies_indexes_2 = pd.DataFrame(anomalies_indexes_2)
    # anomalies_indexes_2.to_csv(r'data/anomalie_2_feature.csv')
